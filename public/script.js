...
fetch('/api/bitcoin-monthly')
    ...
    .then(data => renderChart(data, 'bitcoinChartContainer', 'Bitcoin Monthly Candle Chart'))
...
fetch('/api/ethereum-monthly')
    .then(response => response.json())
    .then(data => renderChart(data, 'ethereumChartContainer', 'Ethereum Monthly Candle Chart'))
    .catch(error => console.error('Error fetching Ethereum data:', error));

// Fetch and render Bitcoin weekly data
fetch('/api/bitcoin-weekly')
    .then(response => response.json())
    .then(data => renderChart2(data, 'bitcoinChartContainer2', 'Bitcoin Weekly Candle Chart'))
    .catch(error => console.error('Error fetching Bitcoin weekly data:', error));
...
function renderChart2(data, containerId, titleText) {
    ...
    const smaData = calculateSMA(data, 10).map((sma, index) => ({
